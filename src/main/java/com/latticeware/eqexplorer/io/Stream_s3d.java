...
public class Stream_s3d
{
    ...
    private File file;
    private FileHeader_s3d header;
    private TreeMap<Integer, DirectoryEntry_s3d> directoryMap;


    public DirectoryEntry_s3d getEntry( Integer _index )
    {
        return directoryMap.get( _index );
    }
    
    
    public Collection<DirectoryEntry_s3d> getEntries()
    {
    ...
    throws IOException
    {
        ...
        header = new FileHeader_s3d();
        
        byte[] _buffer = new byte[ 4 ];
        
        try (final RandomAccessFile _raf = new RandomAccessFile( file, "r" ))
        {
            _raf.read( _buffer, 0, 4 );
            header.offset = Munger.fourBytesToInt( _buffer );
            _raf.read( _buffer, 0, 4 );
            header.cookie = new String( _buffer );
            _raf.read( _buffer, 0, 4 );
            header.magicValue = Munger.fourBytesToInt( _buffer );
            LOG.log( Level.INFO, "{0}", header );
            _raf.seek( header.offset );
            //            Integer _numEntries = _raf.readUnsignedShort();
            _raf.read( _buffer, 0, 4 );
            Integer _numEntries = Munger.fourBytesToInt( _buffer );
            
            LOG.log( Level.INFO, "ArchiveHeader : {0} : {1} : {2}", new Object[ ]{ file.length(), header.offset, _numEntries } );
            ...
            for ( int _i = 0; _i < _numEntries; _i++ )
            {
                ...
                directoryMap.put( _dataOffset, _entry );
            ...
            DirectoryEntry_s3d _fileListing = directoryMap.lastEntry().getValue();
            //            try
            //            {
            ...
            //            }
            Iterator<DirectoryEntry_s3d> _dirIter = directoryMap.values().iterator();
...
//            String _path = "/tmp";
//            for ( int _i : directoryMap.navigableKeySet() )
//            {
//                if ( directoryMap.get( _i ).fileName != null )
//                {
//                    File _f = new File( _path + "/" + directoryMap.get( _i ).fileName );
//                    FileOutputStream _fos = new FileOutputStream( _f );
//                    _fos.write( directoryMap.get( _i ).rawData );
//                    _fos.close();
//                }
//            }
        }
